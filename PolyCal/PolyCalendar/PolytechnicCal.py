...
class LoginPage(ctk.CTkFrame):
    ...
    def register_student(self):
        ...
        registration_window.title("Register Student ID")
        registration_window.geometry("400x400")
        registration_window.configure(bg="#f8f7f2")
        registration_window.transient(self)
        ...
        program_entry = ctk.CTkEntry(
            registration_window,
            placeholder_text="Program (e.g., BSIT 1-2)",
            font=("Arial", 12, "bold"),
            width=200
        )
        ...
        def upload_image():
            ...
            if file_path:
                ...
                image_label.configure(text=f"Selected: {os.path.basename(file_path)}")
        ...
        def save_registration():
            ...
            new_program = program_entry.get().strip()
            new_image_path = image_path_var.get()

            if not new_id or not new_name or not new_program:
                messagebox.showerror("Error", "All fields are required!")
                return

            if any(student['id'] == new_id for student in STUDENT_INFO):
        ...
        if student:
            try:
                ...
                if os.path.isfile(image_path):
                    image = Image.open(image_path)
                    background = Image.new('RGBA', (100, 100), '#f8f7f2')
                    ...
                    default_image = Image.open("C:/Users/acer/Desktop/PolyCal/upload.png")
                    mask = Image.new("L", (100, 100), 0)
                    draw = ImageDraw.Draw(mask)
                    draw.ellipse((0, 0, 100, 100), fill=255)
                    
                    circular_image = ImageOps.fit(default_image, (100, 100), centering=(0.5, 0.5))
                    circular_image.putalpha(mask)
                ...
                # Use default profile picture if image loading fails
                default_image = Image.open("C:/Users/acer/Desktop/PolyCal/upload.png")
                mask = Image.new("L", (100, 100), 0)
                draw = ImageDraw.Draw(mask)
                draw.ellipse((0, 0, 100, 100), fill=255)
