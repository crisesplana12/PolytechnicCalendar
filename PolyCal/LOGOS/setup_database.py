...
def update_database():
    ...
    # Add the deleted column to the archive table if it doesn't exist
    cursor.execute('''
        ALTER TABLE archive ADD COLUMN deleted INTEGER DEFAULT 0
    ''')
    
    # Add the deleted column to the events table if it doesn't exist
    cursor.execute('''
